  ...
  return (
   <div className='h-[88vh] overflow-y-auto'>
    ...
    {searchedResults?.length !== 0 && searchedResults.map((searchedResult) => {
       return  searchedResult?.id?.channelId?<div className='flex justify-center mb-10 border-b-[1px] pb-5' key={searchedResult.etag}>
       ...
       </div>: <NavLink to={'/watch?v=' + searchedResult?.id?.videoId} key={searchedResult.etag} onClick={() => sessionStorage.setItem(SESSION_STORAGE_KEY.VIDEO_DESCRIPTION, JSON.stringify(searchedResult))}>
